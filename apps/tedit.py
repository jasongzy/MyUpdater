...
def get_tedit_version():
    local_version = file_io.get_exe_version(app.exe_path, use_product_version=True)
...
def init(check_release=True):
    ...
    app.local_version = get_tedit_version()
    ...
    app.version_str_processor = lambda x: x.replace("-beta", ".")
...
def update(verbose=True):
    ...
    if file_io.downloader(app.release_file_url, tmp_file, file_io.get_config("common", "proxy_dict"), verbose=verbose):
        return -1
    if file_io.empty_dir_interact(app.local_dir, True, None, verbose=verbose) != 0:
        return -1
